...
class LSTM_IB_GAN_Model(nn.Module):
    ...
    def build(self, x, eps=1e-6):
        ...
        self.scores = x['labels'].long().squeeze()
        # print(self.scores)
...
def train(textData, LM, i2v=None, model_path=args['rootDir'] + '/LSTM_IB_GAN_ce_mimic3_'+args['date'] +'.pt', print_every=10, plot_every=10,
    ...
    for epoch in range(args['numEpochs']):
        ...
        MSEloss, total_prec, average_accuracy, samplerate = test(textData, G_model, 'dev', max_p)
        ...
        # print("about to run a test after epoch: ")
        # res = test_small(textData, G_model, 'test', max_accu)
        #
        # test2(G_model, test_data)
        ...
        print('Epoch ', epoch, 'loss = ', sum(Glosses) / len(Glosses), sum(Dlosses) / len(Dlosses), 'Valid  mseloss= ', MSEloss, 'prec=',total_prec, 'ave accuracy=',average_accuracy,'sample=',samplerate, 'max prec=',
              max_p)
    ...
    pd.DataFrame(training_stats).to_csv(args['rootDir']+"/training_stats_ce_"+args['model_arch']+args['date']+".csv", index=False)
...
def test(textData, model, datasetname, max_accuracy):
    ...
    with torch.no_grad():
        for batch in textData.getBatches(datasetname):
            ...
            print("output_probabilities", output_probs)
            output_labels = np.argmax(output_probs.cpu(), axis=1).flatten().cpu().numpy()
            ...
            # logger.info(f"true labels are: {true_labels} ")
            tmp_eval_accuracy = np.sum(output_labels == true_labels)
            # logger.info(f"output probs shape: {output_probs.shape}")
            # logger.info(f"output probs: {output_probs}" )
            # logger.info("======================================")
            # logger.info(f"output labels shape: {output_labels.shape} " )
            # logger.info(f"output labels: {output_labels}")
            # logger.info("========================================")
            # logger.info(f"x labels shape: {x['labels'].shape} ")
            # logger.info(f"x labels: {x['labels']} ")
            # logger.info("===========================================")
            # logger.info(f"sampled words shape: {sampled_words.shape}")
            # logger.info("sampled words: ", sampled_words)


            # logger.info(f"tmp eval accuracy is: ", tmp_eval_accuracy)
            eval_acc_total += tmp_eval_accuracy
            nb_eval_steps+=x['labels'].shape[0]

            if not pppt:
                pppt = True
                # logger.info("checking the returned sampled words/rationale")
            ...
            # print("batch correct is: ", batch_correct)
    ...
    return MSEloss, total_prec, avg_accuracy, 0
