...
class LSTM_GMIB_Model(nn.Module):
    ...
    def build(self, x, eps=1e-4):
        ...
        # not entirely sure why they add +2 here?
        # y = F.one_hot(self.classifyLabels, num_classes=args['chargenum'] + 2)
        y = F.one_hot(self.classifyLabels, num_classes=args['chargenum'])
        ...
        KL_origin = torch.mean(
            0.5 * (torch.exp(self.charge_dist_logvar) + self.charge_dist_mu ** 2 - 1 - self.charge_dist_logvar))
        ...
        cla_loss = y[:, :args['chargenum']].float() * torch.log(pred + eps) + (
                    1 - y[:, :args['chargenum']].float()) * torch.log(1 - pred + eps)
